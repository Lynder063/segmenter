...
This script automatically creates a isolated virtual environment (`mac/python/venv`), installs all Python dependencies, and launches the native macOS app.
...
```bash
./mac/python/build_app.sh
```

Output artifacts are generated in `mac/python/dist/`:
- `mac/python/dist/Segmenter.app`
- `mac/python/dist/Segmenter-arm64.dmg` (or `Segmenter-x86_64.dmg`)
