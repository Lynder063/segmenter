...
def query_episodes_with_faiss(videos, vectors_dir):
    ...
    for i, length in enumerate(lengths):
        ...
        # FAISS-GPU only supports NVIDIA CUDA, not AMD ROCm
        # Import GPU backend detection
        import sys as _sys
        _sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
        from gpu import is_nvidia
        ...
        if is_nvidia():
            try:
                ...
                gpu_index.add(rest)
                scores, indexes = gpu_index.search(query, k)
                used_gpu = True
            except Exception:
                pass
        
        if not used_gpu:
            # CPU fallback (also used for AMD ROCm)
            index.add(rest)
...
def detect(video_dir, feature_vector_function="CH", annotations=None, artifacts_dir=None, framejump=3, percentile=10,
    ...
    # Set default artifacts_dir to /tmp/introstamp_rcd_cache for automated cleanup
    if artifacts_dir is None:
        artifacts_dir = "/tmp/introstamp_rcd_cache"
